...
:warning: chaque partie contient ses propres tests 

- intro
- trucs de base
- extensions
    - etendre le language
        - opérateurs booléan Insha'Allah _/
        - boucle for _/
        - if sans else _/
        - main (mettre un lien symbolique vers la partie interpret) _/
        - potentiel type string
    - error handling
        - truc de base (genre missing semicolumn)
        - auto-suggestion (variables & fonctions)
    - interprète 
        - mettre a jour cette partie si type string integré sinon _/
-
...
Dans ce projet, nous avons construit la partie frontend d'un compilateur pour un noyau
de C: Mini-C. Nous avons ensuite implémenté les extensions suivantes :
...
Nous avons complété le squelette fournis avec un lexeur, un parseur et un vérificateur de type pour chaque élément de notre language. Nous avons donc complété l'implémentation de la déclaration de variables, de la déclaration de fonctions. Nous avons aussi complété l'implémentation des conditionnelles et des boucles while. A chaque fois, le vérificateur de texte vérifie récursivement le type de nos objets. Par exemple, pour le while :
```ocaml=
...
...
...
```
Dans cet exemple, on cherche le type et la valeur de la variable v stockés dans un environnement local. On verifie que le type de la variable est bien le type de sa valeur. Si le nom de la variable n'est pas une clé de l'environnement, alors le nom est mal écris ou n'existe pas, auquel cas on fait un appel à la fonction `did_u_mean` présentée dans la partie [Error Handling](#Did-you-mean-?).
Les fichiers `test.mnc` et `test_while.mnc` présentent tous les éléments du noyau de base de Mini-C.
...
#### Opérateurs Booléens
:page_facing_up: `test_op_bool.mnc`
...
#### Boucle `for`
:page_facing_up: `test_for_loop.mnc`
...
#### `If` sans `Else`
:page_facing_up: `test_if_without_else.mnc`
...
#### Missing semicolon ?
:page_facing_up: `test_)_expected.mnc` [`test_missing_semicolumn.mnc`](./tests/extensions/test_missing_semicolumn.mnc)
...
#### Did you mean ?
:page_facing_up: `test_did_u_mean_var.mnc` `test_did_u_mean_fun.mnc`
...
Ainsi si 2 variables, 1 globale et 1 locale ont le même distance de levenshtein, c'est l'identifiant de la varibale locale qui sera renvoyer.
